...
class CartMixin: # получение корзины
...
class CartModalView(CartMixin, View): # представление для модального окна
...
class AddToCartView(CartMixin, View): # добавление в корзину
    @transaction.atomic
...
class UpdateCartItemView(CartMixin, View): # обновление количества товаров
    @transaction.atomic
...
class RemoveCartItemView(CartMixin, View): # удаление товара
    def post(self, request, item_id):
        cart = self.get_cart(request)
...
class CartCountView(CartMixin, View): # количество товара в корзине на данный момент
    def get(self, request):
        cart = self.get_cart(request)
        return JsonResponse({
            'total_items': cart.total_items,
...
class ClearCartView(CartMixin, View): # удаление всех товаров из корзины
...
class CartSummaryView(CartMixin, View): # вывод товара в корзине
    def get(self, request):
        cart = self.get_cart(request)
        context = {
            'cart': cart,
